...
function likes_enqueue_scripts() {
	wp_enqueue_script( 'defier-like-but', plugin_dir_url( __FILE__ ) .  '/js/likes.js', array( 'jquery' ), '0.5', false );
	wp_localize_script( 'defier-like-but', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'like' => __( 'Like', 'defier' ),
		'unlike' => __( 'Unlike', 'defier' )
	) );
}
...
function process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( __( 'Not permitted', 'defier-magazine-blog' ) );
	}
 ...
 * @since    0.5
 */
function defier_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( 'Unlike', 'defier-magazine-blog' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( 'Like', 'defier-magazine-blog' );
		$icon = $icon_empty;
	}
...
function sl_shortcode() {
	return defier_likes_button( get_the_ID(), 0 );
} // shortcode()
...
function get_liked_icon() {
	...
	$icon = '<span class="sl-icon"><i class="fa-heart2"></i></span>';
	return $icon;
} // get_liked_icon()
...
function get_like_count( $like_count ) {
	$like_text = __( 'Like', 'defier-magazine-blog' );
...
function show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'defier-magazine-blog' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php _e( 'You do not like anything yet.', 'defier-magazine-blog' ); ?></p>
			<?php
			endif;
			wp_reset_postdata();
			?>
