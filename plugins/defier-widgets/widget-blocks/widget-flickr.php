<?php
add_action('widgets_init','Defier_Flickr_widget');

function Defier_Flickr_widget() {
	register_widget('Defier_Flickr_widget');
	
	}
class Defier_Flickr_widget extends WP_Widget {
	public function __construct() {
		parent::__construct( 'quick-flickr-widget', 'Defier Widget - Flickr ', array(
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $title ) )
			echo "<span class='defier-cat-title'><h4 class='widget-title'><div class='lavecat flickrLabel'></div>" . $title ." </h4></span>";
		...
		if ( is_wp_error( $photos ) ) {
			...
			foreach ( $photos as $photo ) {
				...
				$item = sprintf( '<div class="quick-flickr-item col-lg-4 col-sm-2 col-md-4 col-xs-4">%s</div>', $item );
				echo $item;
			}
		}
