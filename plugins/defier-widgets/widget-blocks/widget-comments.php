...
/********* Comment Widget  ***********/
/*************************************/
add_action('widgets_init','Defier_Comments_Widget');

function Defier_Comments_Widget() {
	register_widget('Defier_Comments_Widget');
	
	}
class Defier_Comments_Widget extends WP_Widget
{
	public function __construct()
	{
		parent::__construct(
			'defier-latest-comments-widget',
			'Defier widget - Recent Comments',
	...
	public function widget($args, $instance) 
	{
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Latest Comments', 'defier-widgets') : $instance['title'], $instance, $this->id_base);
		if (empty($instance['number']) || !$number = absint($instance['number'])) {
 			$number = 5;
		}
		
		$query = get_comments(apply_filters('defier_widget_comments_query_args', array('number'=> $number, 'status' => 'approve')));
		
		// do custom loop if available
		if (has_action('defier_widget_comments_loop')):

			$args['title'] = $title;
			do_action('defier_widget_comments_loop', array_merge($args, $instance), $query);
...
?>
		...
		set_transient('defier_widget_comments', $cache);
	}
	...
	{
		delete_transient('defier_widget_comments');
	}
...
?>
		...
		<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of comments:', 'defier-widgets'); ?></label>
