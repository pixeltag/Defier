...
Author URI: http://www.Defier.biz/
License: GPL2+
*/

class Defier_Category_Colors{
    protected $_meta;
...
wp_enqueue_style( 'wp-color-picker' );
    ...
    function check_field_color(){
        ...
        wp_enqueue_style( 'wp-color-picker' );
        $this->js .= '$("#rl_cat_color").wpColorPicker();';
    }
    ...
    function show( $tag, $taxonomy ){
        ...
        wp_nonce_field( basename( __FILE__ ), 'Defier_taxonomy_meta_nonce' );

        echo "<table class='form-table'>";
        foreach ( $this->_fields as $field ) {
            echo '<tr>';

            $meta = !empty( $metas[$field['id']] ) ? $metas[$field['id']] : $field['id'];
    ...
    function save( $term_id, $tt_id ) {
        ...
        foreach ( $this->_fields as $field ) {
            $name = $field['id'];
            $new = isset( $_POST[$name] ) ? $_POST[$name] : ( $field['id'] ? array() : '' );
