...
//                }
            ...
            public static function compileSystemStatus( $json_output = false, $remote_checks = false ) {
                ...
                $sysinfo['redux_ver']      = __( ReduxFramework::$_version );
                ...
                $sysinfo['server_info'] = do_shortcode( $_SERVER['SERVER_SOFTWARE'] );
                $sysinfo['localhost']   = self::makeBoolStr( self::isLocalHost() );
                $sysinfo['php_ver']     = function_exists( 'phpversion' ) ? do_shortcode( phpversion() ) : 'phpversion() function does not exist.';
                    ...
                    foreach ( $files as $file ) {
                        ...
                        if ( $theme_file ) {
                            ...
                            if ( $core_version && ( empty( $theme_version ) || version_compare( $theme_version, $core_version, '<' ) ) ) {
                                ...
                                $found_files[ $plugin_name ][] = sprintf( __( '<code>%s</code> version <strong style="color:red">%s</strong> is out of date. The core version is %s', 'defier' ), str_replace( WP_CONTENT_DIR . '/themes/', '', $theme_file ), $theme_version ? $theme_version : '-', $core_version );
                            } else {
